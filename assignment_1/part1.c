...
node* new_node() {
    ...
    return n; // Return the newly allocated node pointer
}
...
void * thread_func (void *arg) {
    ...
    for (int i = 0; i < targ->count; i++) {
        pthread_mutex_lock(&tree_lock); // Acquire the lock before accessing the tree
        ...
        pthread_mutex_unlock(&tree_lock); // Release the lock after accessing the tree
    }
}
